 ...
 */
const productDao = require('../../models/dao/admin/productDao');
module.exports = {
  /**
   * 获取商品图片
   * @param {Object} ctx
   */
  GetProductImg: async ctx => {
    let { product_id } = ctx.request.body;
    const pictures = await productDao.GetProductImg(product_id);
  ...
  GetAllProduct: async ctx => {
    let { page, limit, fName, cName } = ctx.request.body;
    // 计算开始索引
    const offset = (page - 1) * limit;
    const Product = await productDao.GetAllProduct(offset, limit, fName, cName);
    const category = await productDao.GetCategory();
    // 获取所有的商品数量,用于前端分页计算
